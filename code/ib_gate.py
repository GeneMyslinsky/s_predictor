...
from ibapi.common import OrderId, ListOfContractDescription, BarData,\
        HistogramDataList, TickerId,ListOfHistoricalTick
...
class IBApp(IBWrapper, IBClient):
    ...
    def request_ticks(self,symbol):
        ...
        self.reqHistoricalTicks(18001, contract,"20200827 11:39:33", "", 10, "TRADES", 0,True, [])
        #self.reqHistoricalTicks(18001, contract,"20200828 11:39:33", "", 1000, "TRADES", 1, True, [])
        # Wait until the request has returned (make it blocking).
        start_time = datetime.now()
        print('started loop')
        while self._historical_ticks is not True:
            if (datetime.now() - start_time).microseconds > 1000000:
                raise HistoricalRequestError(
                    "Timeout occurred while retrieving price data for {}"
                    .format(symbol),
            ...
            print(f'1r {(datetime.now() - start_time).microseconds}')
            time.sleep(0.2)
            print(self._historical_ticks)
